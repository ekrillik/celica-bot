...
class Beginners(commands.Cog):
    ...
    async def roadmap(self, ctx: commands.Context[commands.Bot]):
        ...
        embed.set_image(url="https://pgr-discord-bot.s3.ap-southeast-2.amazonaws.com/Infographics/NEW_PGR_ROADMAP-9.png")
        await ctx.send(embed=embed)
    ...
    async def damageformulas(self, ctx: commands.Context[commands.Bot]):
        ...
        embed.add_field(name=f"Physical Damage (Non-Crit) - **White Numbers**/Slash Affix(Non-Crit) - **Orange Numbers**", value=f"\nSkill Scaling Percentage x (Base ATK x (1 + ATK Increase) + Final ATK) x (1 + Phys DMG Increase) x (1 + Base DMG Increase) x (1 + Extra DMG Increase) x (1 - New DEF Stat) x (1 - Physical Resistance) x (1 - Extra DMG Reduction) x Float = Final DMG Output", inline=False)
        embed.add_field(name=f"Physical Damage (Crit) - **Yellow Numbers**/Slash Affix(Crit) - **Orange Numbers**", value=f"\n(2 + Crit DMG Increase) x Skill Scaling Percentage x (Base ATK x (1 + ATK Increase) + Final ATK) x (1 + Phys DMG Increase) x (1 + Base DMG Increase) x (1 + Extra DMG Increase) x (1 - New DEF Stat) x (1 - Physical Resistance) x (1 - Extra DMG Reduction) x Float = Final DMG Output", inline=False)
        embed.add_field(name=f"Elemental - **White Numbers with elemental icon**/Affix Damage (does not include Slash) - **Orange Numbers with affix icon**", value=f"\nSkill Scaling Percentage x (Base ATK x (1 + ATK Increase) + Final ATK) x (1 + Elemental DMG Increase) x (1 + Base DMG Increase) x (1 + Extra DMG Increase) x (1 - Elemental Resistance Resistance) x (1 - Extra DMG Reduction) = Final DMG Output", inline=False)
        await ctx.send(embed=embed)
